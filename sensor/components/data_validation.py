...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        """
        This function will drop columns which conatins missing value more than specified threshold
        ...
        try:
            ...
            #selecting columnns names which contains null values
            logging.info(f"selecting columnns names which contains null values above to {threshold}")
            ...
            logging.info("Dropped all colummns and return None if no columns are dropped")
            if len(df.columns)==0:
                return None
            return df
        ...
        try:
            ...
            for base_column in base_columns:
                if base_column not in current_columns:
                    logging.info(f"Columns: [{base_column} is not avaialble]")
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            logging.info("Drop null values column from train df")
            train_df=self.drop_missing_values_columns(df=train_df,report_key_name="missing_values_within_train_dataset")
            logging.info("Drop null values column from test df")
            ...
            if train_df_columns_status:
                logging.info("As all columns are available in train df hence detecting data drift in train df")
                self.data_drift(base_df=base_df, current_df=train_df,report_key_name="data_drift_within_train_dataset")
            if test_df_columns_status:
                logging.info("As all columns are available in train df hence detecting data drift in test df")
                self.data_drift(base_df=base_df, current_df=test_df,report_key_name="data_drift_within_test_dataset")


            # write the report
            logging.info("write a report in yamal file")
